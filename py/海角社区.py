...
# 播放通路：详情附件中的视频地址为 30 秒预览清单，真实分片文件为站点公开资源。
...
class Spider(_BaseSpider):
    ...
    def categoryContent(self, tid, pg, filter=False, extend=None):
        ...
        if key in ("new", "latest", ""):
            ...
            data = self._api("/topic/node/topics", {"node_id": key, "page": page, "limit": PAGE_SIZE})
    ...
    def _key_material(self, preview_url, referer):
        ...
        raw_key = None
        if key_uri:
            raw_key = self._fetch_bytes(key_uri, referer=referer, timeout=10)
        salt = None
        try:
            salt_url = preview_url.replace(".m3u8", ".jpg")
            if salt_url != preview_url:
                salt_raw = self._fetch_bytes(salt_url, referer=referer, timeout=10)
                if salt_raw:
    ...
    def _seg_exists(self, url, referer):
        try:
    ...
    def _probe_last_index(self, base_dir, prefix, start, estimate, referer, seed_count):
        ...
        def probe_many(indexes):
            ...
            try:
                ...
                with ThreadPoolExecutor(max_workers=8) as pool:
                    flags = list(pool.map(lambda i: self._seg_exists(seg_url(i), referer), targets))
            except Exception:
                flags = [self._seg_exists(seg_url(i), referer) for i in targets]
            return targets, flags

        center = max(int(estimate or 0), int(seed_count or 0), start + 1)
        coarse = [center + off for off in range(-180, 201, 20)]
        targets, flags = probe_many(coarse)
        low, high = start, None
        for idx, ok in zip(targets, flags):
            if ok:
                low = idx
            else:
                high = idx
        ...
        if high is None:
            probe = low
            step = max(int(center * 0.25), 100)
            for _ in range(8):
                probe += step
                if self._seg_exists(seg_url(probe), referer):
                    low = probe
                    step = int(step * 1.4) + 40
                else:
                    high = probe
                    break
            if high is None:
                high = low + 1
        if high - low > 5:
            fine = [low + 4 * k for k in range(1, min((high - low) // 4, 80) + 1)]
            targets2, flags2 = probe_many(fine)
            for idx, ok in zip(targets2, flags2):
                if ok:
                    low = idx
                else:
                    high = idx
